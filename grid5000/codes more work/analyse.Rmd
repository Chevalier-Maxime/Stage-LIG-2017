...
```
## Expérience
### Augmentation de la quantitée de calcul
...
On va s'intéréssé à l'augmentation de la charge de calcul pour chaque processus dans un premier temps :
...
ggplot(df60, aes(x=size,y=runningTime, color=sim, shape= factor(nbProcess))) +geom_point() + aes(ymin=0) +xlab("Taille des traces simulées")+ylab("Running Time en seconde (échelle log)") + ggtitle("Temps de simulation AMG2013") + scale_y_log10() + scale_color_discrete(name="Simulation", labels=c("Parallèle", "Sequentielle")) + labs(subtitle="Génération sur 64 noeuds physiques(parallèle), topologie dragonfly 216, grille de calcul d'un processus 60x60x60") + scale_shape_discrete(name=c("Nombre de Processus"), labels=c("1","4"))
```
...
ggplot(df80, aes(x=size,y=runningTime, color=sim, shape= factor(nbProcess))) +geom_point() + aes(ymin=0) +xlab("Taille des traces simulées")+ylab("Running Time en seconde (échelle log)") + ggtitle("Temps de simulation AMG2013") + scale_y_log10() + scale_color_discrete(name="Simulation", labels=c("Parallèle", "Sequentielle")) + labs(subtitle="Génération sur 64 noeuds physiques(parallèle), topologie dragonfly 216, grille de calcul d'un processus 80x80x80") + scale_shape_discrete(name=c("Nombre de Processus"), labels=c("1","4"))
```
...
ggplot(df100, aes(x=size,y=runningTime, color=sim, shape= factor(nbProcess))) +geom_point() + aes(ymin=0) +xlab("Taille des traces simulées")+ylab("Running Time en seconde (échelle log)") + ggtitle("Temps de simulation AMG2013") + scale_y_log10() + scale_color_discrete(name="Simulation", labels=c("Parallèle", "Sequentielle")) + labs(subtitle="Génération sur 64 noeuds physiques(parallèle), topologie dragonfly 216, grille de calcul d'un processus 100x100x100") + scale_shape_discrete(name=c("Nombre de Processus"), labels=c("1","4"))
```